---
description: Audits existing game code against plugin patterns — checks server-authority, schema conventions, auth security, payment safety, narrative coherence, and MVP scope drift. Use after building components or before committing.
argument-hint: Optional component name or path to review (defaults to entire src/)
---
...
3. Determine scope from `$ARGUMENTS`:
   - If `$ARGUMENTS` is provided: focus on `src/$ARGUMENTS/` or the path provided
   - If empty: review all of `src/`
4. Scan the target scope to identify which technology domains are present (auth, payments, WebSocket, Redis, BullMQ, quests, etc.)
5. Load only the SKILL.md files relevant to the detected domains from `${CLAUDE_PLUGIN_ROOT}/engineering/`, `${CLAUDE_PLUGIN_ROOT}/design/`, `${CLAUDE_PLUGIN_ROOT}/narrative/`
...
**Skill ref:** [→ engineering-stripe-game-payments: webhook-first fulfillment]
```
...
- Review /game-dev:engineering-[skill] for patterns on [top violation category]
- Run /game-dev:game-build [component] to rebuild any component with critical violations
```
