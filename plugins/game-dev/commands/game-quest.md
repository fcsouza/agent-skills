---
description: Designs a single quest end-to-end — coherence check, objective tree, quest brief, and registry entry. Requires docs/world-lore.md and docs/quest-registry.md.
argument-hint: quest name (e.g. "The Merchant's Debt", "Rescue at the Crossroads")
---

# Game Quest — Quest Designer Command
...
1. READ `docs/world-lore.md`
   - If it does NOT exist: output `⛔ Cannot proceed. docs/world-lore.md not found. Run /game-dev:game-architect first to generate the world foundation, or create docs/world-lore.md manually.` and stop.
2. READ `docs/quest-registry.md`
   - If it does NOT exist: create it using the template at `${CLAUDE_PLUGIN_ROOT}/narrative/quest-narrative-coherence/templates/quest-registry.md`. If that template is missing, create an empty registry with headers: Quest ID | Name | Type | NPC | Location | Factions | Rewards | Prerequisites | Arc.
3. READ `${CLAUDE_PLUGIN_ROOT}/narrative/quest-narrative-coherence/SKILL.md`
4. READ `${CLAUDE_PLUGIN_ROOT}/design/quest-mission-design/SKILL.md`
...
Run the 5-step check from `quest-narrative-coherence` against `$ARGUMENTS` (the proposed quest name/concept):
...
### Economy Note
[Reward amount cross-referenced against mvp-first-draft.md economy params OR flagged as: "⚠️ Economy not yet defined — validate rewards when running /game-dev:game-balance"]
```
...
⚠️ New [NPC/Location] introduced: [name]
   → Run /game-dev:game-lore npc [name] to add them to world-lore.md
```
