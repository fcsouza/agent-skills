---
description: Adds or updates a single world lore entry (faction, location, NPC, or event) — validates against existing lore for consistency, applies the right template, and updates docs/world-lore.md.
argument-hint: type and name (e.g. "faction The Iron Court", "location Ashfall Crossing", "npc Mira the Keeper", "event The Founding War")
---
...
1. READ `${CLAUDE_PLUGIN_ROOT}/narrative/worldbuilding/SKILL.md`
2. READ `${CLAUDE_PLUGIN_ROOT}/narrative/quest-narrative-coherence/SKILL.md`
3. READ `docs/world-lore.md`
   - If it does NOT exist: output `⛔ Cannot proceed. docs/world-lore.md not found. Run /game-dev:game-architect first to generate the world foundation, or create docs/world-lore.md manually with a World Overview, Factions, Locations, Characters, and History sections.` and stop.
4. Parse `$ARGUMENTS` to detect entry type:
   - Starts with "faction" → FACTION mode
   - Starts with "location" → LOCATION mode
   - Starts with "npc" or "character" → NPC mode
   - Starts with "event" or "history" → HISTORICAL EVENT mode
   - Not recognized → ask: "What type of lore entry is this? Reply with: faction / location / npc / event"
5. Extract the entry name from `$ARGUMENTS` (the text after the type keyword)
...
🔗 Quest registry cross-reference:
  ...
  [If quests reference it]: Consider running /game-dev:game-quest to update those quests with the new lore.
```

If `docs/quest-registry.md` doesn't exist:
```
🔗 Quest registry: not yet created — run /game-dev:game-quest to start creating quests that use this lore.
```
