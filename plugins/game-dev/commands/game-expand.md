---
description: Adds a new feature to an existing MVP plan — scoped architect interview, scope validation against original plan, and integration into the build sequence. Run /game-architect first.
argument-hint: feature name or description (e.g. "clan system", "seasonal events", "crafting")
---

# Game Expand — Feature Expansion Command
...
1. READ `docs/mvp-first-draft.md`
   - If it does NOT exist: output `⛔ Cannot proceed. docs/mvp-first-draft.md not found. Run /game-dev:game-architect first to create your MVP plan before expanding it.` and stop.
...
Search `docs/mvp-first-draft.md` Section 10 for anything matching `$ARGUMENTS`:
...
Search `docs/mvp-first-draft.md` Section 9 (Build Sequence) for `$ARGUMENTS`:
- If `$ARGUMENTS` matches a component already IN the MVP plan and in build-registry as "built": output `⚠️ [feature] is already built (see docs/build-registry.md). Did you mean to refactor it? Use /game-dev:game-review [component] to audit the existing implementation.` and stop.
- If `$ARGUMENTS` matches a component already in the plan but NOT built yet: output `⚠️ [feature] is already in your MVP plan (Section 9) and not yet built. Use /game-dev:game-build [component] to implement it instead of re-planning.` and stop.
...
Unlike `/game-dev:game-architect` which runs a full game interview, this command asks ONLY what's needed to spec this specific feature.
...
1. **Append the expansion section** at the end of `docs/mvp-first-draft.md`:
   ...
   > Added: [current date] via /game-dev:game-expand
...
Next step:
  /game-dev:game-build [first-unbuilt-dependency-or-new-component]
```
